...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function updateMainContent(newHtml) {
        ...
        if (typeof window.GrafikPicture === 'object' && typeof window.GrafikPicture.refresh === 'function') {
            window.GrafikPicture.refresh(); // Refresh picture if grafik.html is loaded
        }
    }
...
const PictureUtils = (() => {
    ...
    const validateImageFile = (file) => {
        ...
        if (!ALLOWED_FORMATS.includes(file.type)) {
            return { valid: false, error: `Invalid file format. Allowed formats: JPG, PNG, GIF, WebP` };
        }
    ...
    /**
     * Retrieve picture from localStorage
    ...
    const getPictureFromStorage = () => {
        try {
            ...
            console.error('Error retrieving picture from storage:', e);
            return null;
        }
    };

    /**
     * Delete picture from localStorage
     * @returns {Object} - {success: boolean, error: string|null}
     */
    const deletePictureFromStorage = () => {
        try {
            ...
            return { success: false, error: `Error deleting picture: ${e.message}` };
        }
    };

    /**
     * Get storage change event for cross-tab communication
     * @param {Function} callback - Function to call when picture changes
     */
    const onPictureChanged = (callback) => {
